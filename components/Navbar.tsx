...
const Navbar: React.FC = () => {
  ...
  return (
    <header className={`fixed top-6 left-0 right-0 z-50 flex justify-center px-4 transition-all duration-300 ${scrolled ? 'top-2' : 'top-6'}`}>
      <div className={`bg-white/90 backdrop-blur-md border border-slate-200/60 rounded-full px-8 py-3 flex items-center justify-between shadow-lg max-w-5xl w-full transition-all ${scrolled ? 'py-2' : 'py-3'}`}>
        <div className="flex items-center">
          <h2 className="text-xl font-black tracking-tight text-primary">Global Horizon</h2>
        </div>
